...
const traverse: typeof _traverse.default = ((_traverse as any).default ??
  _traverse) as any;
const generate: typeof _generate.default = ((_generate as any).default ??
  _generate) as any;
...
function cdnPrefixImages(): Plugin {
  ...
  const rewriteJsxAst = (code: string, id: string, cdn: string) => {
    ...
    traverse(ast, {
      JSXAttribute(path) {
        ...
        if (t.isStringLiteral(val)) {
          ...
          val.value = isSrc
            ? toCDN(val.value, cdn)
            : rewriteSrcsetList(val.value, cdn);
    ...
    const out = generate(
      ast,
      { retainLines: true, sourceMaps: false },
      code
    ).code;
